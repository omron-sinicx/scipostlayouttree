...
class DetrDatasetMapper:
    ...
    def __call__(self, dataset_dict):
        ...
        if "annotations" in dataset_dict:
            ...
            keep = instances.gt_boxes.nonempty()
            assert keep.all()
            instances = instances[keep]
            kept_annos = [a for a, k in zip(annos, keep.tolist()) if k]

            instances.gt_priority = torch.tensor(
                [a["priority"] for a in kept_annos], dtype=torch.int64
            )
            instances.gt_idx = torch.tensor(
                [a["idx"] for a in kept_annos], dtype=torch.int64
            )
            instances.gt_parents  = torch.tensor(
                [a["parents"]  for a in kept_annos], dtype=torch.int64
            )
            # instances.gt_parents  = torch.tensor(
            #     [a["children"]  for a in kept_annos], dtype=torch.int64
            # )
            instances.gt_next = torch.tensor(
                [a["next"] for a in kept_annos], dtype=torch.int64
            )
            instances.gt_depth = torch.tensor(
                [a["depth"] for a in kept_annos], dtype=torch.int64
            )
            instances.gt_text_input_ids = torch.tensor(
                [a["text_input_ids"] for a in kept_annos], dtype=torch.int64
            )
            instances.gt_text_attention_mask = torch.tensor(
                [a["text_attention_mask"] for a in kept_annos], dtype=torch.int64
            )
