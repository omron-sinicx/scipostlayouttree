...
from ...relation.modeling.drgg import DRGGHead
...
class DINO(nn.Module):
    ...
    def __init__(
        ...
        num_feature_levels = len(self.neck.convs)
        if self.neck.extra_convs is not None:
            num_feature_levels += len(self.neck.extra_convs)
        self.relation_head = DRGGHead(
            ...
            num_stages=self.transformer.decoder.num_layers,
    ...
    def forward(self, batched_inputs):
        ...
        if dn_meta is not None:
            dn_padding = dn_meta["single_padding"] * dn_meta["dn_num"]
        else:
            dn_padding = 0
        box_feats_list = [
            [inter_states[lvl][i, dn_padding:] for lvl in range(inter_states.shape[0])]
            for i in range(inter_states.shape[1])
        ]

        if self.training:
            _, rel_loss = self.relation_head(
                fpn_feats=multi_level_feats,
            ...
            processed_results, _ = self.relation_head(
                fpn_feats=multi_level_feats,
                box_feats_list=box_feats_list,
                proposals=processed_results
            )
