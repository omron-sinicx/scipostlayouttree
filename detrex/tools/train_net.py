...
def do_test(cfg, model, eval_only=False):
    ...
    if eval_only:
        ...
        if comm.is_main_process():
            run = wandb.init(
                project="scipostlayouttree",
                name=os.path.basename(cfg.train.output_dir)+"_test",
                tags=["test"],
                config=dict(cfg),
            )
            ...
            wandb.log(_flatten(ret, prefix="test"), step=0, commit=True)
...
def do_train(args, cfg):
    ...
    # load before creating DDP model
    if cfg.train.load_backbone_only == True:
        # Load initial checkpoint into backbone
        if cfg.train.init_checkpoint:
            try:
                ckpt = torch.load(cfg.train.init_checkpoint, map_location="cpu")
                state_dict = ckpt.get("model", ckpt)

                if hasattr(model.backbone, "bottom_up"):
                    # DiT, LayoutLMv3
                    if hasattr(model.backbone.bottom_up, "backbone"):
                        backbone_module = model.backbone.bottom_up.backbone
                        if "LayoutLMv3" in type(backbone_module).__name__:
                            stripped_state_dict = {
                                k.replace("layoutlmv3.", "", 1): v
                                for k, v in state_dict.items()
                            }
                        else:
                            stripped_state_dict = state_dict
                    # InternImage, Swin, etc.
                    else:
                        backbone_module = model.backbone.bottom_up
                        stripped_state_dict = {
                    ...
                    backbone_module = model.backbone.net
                    stripped_state_dict = {
                        k.replace("backbone.net.", "", 1): v
                        for k, v in state_dict.items()
                        if k.startswith("backbone.net.")
                    }

                missing, unexpected = backbone_module.load_state_dict(stripped_state_dict, strict=False)
                logger.info(f"Loaded backbone checkpoint from {cfg.train.init_checkpoint}")
                logger.info(f"Missing keys: {len(missing)}")
                logger.info(f"Unexpected keys: {len(unexpected)}")
            except Exception as e:
                logger.warning(f"Failed to load init_checkpoint into backbone: {e}")
    ...
    if cfg.train.load_backbone_only == False:
...
def main(args):
    ...
    cfg.train.wandb.params.dir = cfg.train.output_dir
    cfg.train.wandb.params.project = "scipostlayouttree"
    cfg.train.wandb.params.name = os.path.basename(cfg.train.output_dir)

    # dump the testing results into output_dir for visualization
    for evaluator in cfg.dataloader.evaluator:
        evaluator.output_dir = cfg.train.output_dir
        if "TreeEvaluator" in str(evaluator._target_):
            evaluator.beam_width = cfg.model.beam_width if hasattr(cfg.model, "beam_width") else 1
